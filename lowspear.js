...
const client = new Discord.Client({restTimeOffset: 100,messageCacheMaxSize: 5});
const { Client, Util } = require("discord.js");
const fs = require("fs");
require("./util/eventLoader")(client);

client.ayarlar = { 
"token": "NzQ3MzY4MDU3OTY5NTczOTUx.X0N2vg.v24eytKSYw4zHXmphFp5R7Qnh1E", // token
"prefix": "gh!", // prefix
"sahip": "508606474608574474",// sahip
}
...
setInterval( async () => {
  ...
  let music = await client.shard.fetchClientValues("voice.connections.size")
  let embed = new Discord.MessageEmbed()
  let i = client.shard.ids
  let pingmesaj;
  let pingdurum;
if(client.ws.ping < 110){
...
pingdurum = "#66ff00"
  ...
  const kanal = db.get("istatistik.kanal");
  const mesajid = db.get("istatistik.msg");
  let botkanal = client.channels.cache.find(c => c.id === kanal);
   if (botkanal) {
  for(i=0;i<client.shard.count;i++) {
     
  embed.setTitle("CrossPlex Bot Status", client.user.displayAvatarURL()).setTimestamp().setColor(pingdurum).setFooter(`Bu İstatistik Dakikada Bir Güncellenir`)
      	.setURL('http://gg.gg/CrossPlex')
 .setDescription(`Bot İle İlgili Tavsiyelerinizi Veya Bulduğunuz Hataları \`${client.ayarlar.prefix}bug\` Yazarak Bize İletebilirsiniz.`)
.addField(`**Shard:** ${i+1}`, `» **Sunucular:** ${server[i]}\n» **Kullanıcılar:** ${user[i]}\n» **Müzik Çalınan Sunucu**: ${music[i]}\n» **Gecikme:** ${Math.round(ping[i])}ms ${pingmesaj}\n» **Bellek:** ${(process.memoryUsage().heapUsed / 1024 / 442).toFixed(2)} MB\n» **İşlemci/Node:** ${(process.cpuUsage().system / 1024 / 442).toFixed(2)}/${(process.cpuUsage().user / 1024 / 442).toFixed(2)}\n» **Çalışma:** ${moment.duration(uptime[i]).format(`D [Gün] , H [Saat], m [Dakika], s [Saniye]`)}\n\n**» Linkler:**\n[(Bot Davet)](https://discord.com/oauth2/authorize?client_id=825858354797871134&permissions=8&redirect_uri=https%3A%2F%2Fdiscord.gg%2FsGWEGY7aPu&scope=bot) [(Destek Sunucu)](https://discord.gg/sGWEGY7aPu)`, true)
    embed.setThumbnail(client.user.displayAvatarURL({ dynamic: true, format: "png", size: 1024 }))
  }
  botkanal.messages.fetch(mesajid).then(m => m.edit(embed));
      }
 } 
   }, 60000) 
 });
